...
st.title("법률 문서 임베딩 기반 유사 판례 검색")
st.caption("Sentence-BERT 임베딩과 코사인 유사도를 활용한 교육용 판례 검색 시스템")
...
with st.sidebar:
    ...
    try:
        ...
        if using_saved_index:
            st.info("저장된 임베딩 인덱스를 사용합니다.")
        else:
            st.info("공개 시연용 합성 샘플 데이터를 사용합니다.")
...
if using_saved_index:
    st.info(
        "로컬 실험 환경에서는 저장된 인덱스를 사용할 수 있습니다. "
        "GitHub 공개 저장소에는 데이터 이용 조건상 샘플 데이터만 포함됩니다.",
        icon="ℹ️",
    )
else:
    st.warning(
...
st.divider()
st.caption(
    "실험 및 모델 학습에는 AI Hub 법률 데이터가 사용되었으나, "
    "원본 데이터와 가공 데이터, 모델 파일, 임베딩 인덱스는 저장소에 포함하지 않습니다."
)
